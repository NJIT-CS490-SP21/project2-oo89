...
const App = () => {
  ...
  function leaderboardSHClick() {
    setShownLB((prevShownLB) => !prevShownLB);
  }
  ...
  useEffect(() => {
    console.log(`Winner--${winner}`);
    if (winner === 'X') {
      const [winnerName, loserName] = activeUsers;
      ...
      <div>
        {isShown === false ? (
          <div className="loginD">
            ...
            <button type="button" className="loginButton" onClick={() => loginClick()}>
              {' '}
              Login
              {' '}
            </button>
        ...
        <div>
          {activeUsers.map((item, index) => (
            <li>
              {item}
              {' '}
              {index === 0 ? 'X' : index === 1 ? 'O' : 'spectator'}
              {' '}
            </li> // if index ==0 print x else if index==1 print o else spec
          ))}
        </div>
