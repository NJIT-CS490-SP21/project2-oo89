    ...
    def test_success(self):
        for test in self.success_test_params:
            with patch('models.Person.query') as mocked_query:
                ...
                self.assertEqual(len(actual_userLResult), len(expected_scoreLResult))
                self.assertEqual(len(actual_scoresLResult), len(expected_userLResult))
                self.assertEqual(actual_userLResult[0], expected_userLResult[0])
                self.assertEqual(actual_userLResult[1], expected_userLResult[1])
    ...
    def test_failure(self):
        for test in self.failure_test_paramus:
            with patch('models.Person.query') as mocked_query:
                ...
                self.assertNotEqual(len(actual_scoresLResult), len(scoreLResult2))
